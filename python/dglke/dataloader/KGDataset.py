...
def _get_id(dict, key):
    id = dict.get(key, None) # If the key is not found in the dictionary, the get() method returns the default value None.
    if id is None: 
        id = len(dict) # add a new key-value pair in the dictionary, id is maxid+1
        dict[key] = id 
    return id

def _parse_srd_format(format):
    if format == "hrt": # generally we use only hrt
        return [0, 1, 2]
    if format == "htr":
        return [0, 2, 1]
    if format == "rht":
...
class KGDataset:
    ...
    The folder with a knowledge graph has six files:
    ...
    * entity_class_map stores each entity and its class type.
    * train_class_map stores each train triple and its class type by head.
    ...
    def __init__(self, entity_path, relation_path, train_path,
                 valid_path=None, test_path=None, train_class_path=None, format=[0,1,2],
        ...
        # if we have the class type files
        """
        if type_path is not None:
            self.entity2classe, self.n_entity2classe = self.read_type(type_path)
        else:
            self.entity2classe = None
        """

        # train triple with class file
        if train_class_path is not None:
            self.train_with_class = self.read_triple_class(train_class_path, "train", skip_first_line)

        else:
            self.train_with_class = None

    # added!!
    """
    def read_type(self, type_path):
        with open(type_path) as f: 
            entity2classe = {} # key is entity id; value is its class id
            for line in f: # for each eid \t e \t classid
                eid, e, classid = line.strip().split(self.delimiter)
                entity2classe[eid] = int(float(classid))

        return entity2classe, len(entity2classe) # return the dictionary and its length
    """

    def read_entity(self, entity_path):
        with open(entity_path) as f: # A Python statement that opens a file and assigns a file object to the variable f.
            entity2id = {} # A dictionary is a collection of key-value pairs, store and access values by a unique key.
            for line in f: # for each id \t entity
                eid, entity = line.strip().split(self.delimiter)
                entity2id[entity] = int(eid) # key is entity, value is its id, dictionary can find quickly the id of each entity, O(1)

        return entity2id, len(entity2id) # return the dictionary and its length
    ...
    def read_relation(self, relation_path):
        with open(relation_path) as f:
            ...
            for line in f:
                ...
                relation2id[relation] = int(rid) # key is relation, value is its id, dictionary can find quickly the id of each relation, O(1)
    ...
    def read_triple(self, path, mode, skip_first_line=False, format=[0,1,2]):
        ...
        print('Reading {} triples....'.format(mode)) # e.g. Reading train triples...
        heads = []
        tails = []
        rels = []
        with open(path) as f:
            ...
            for line in f:
                ...
                heads.append(self.entity2id[h]) # self.entity2id is already called before self.read_triple
                rels.append(self.relation2id[r]) # so we can get the id of the head/relation/tail
                tails.append(self.entity2id[t]) # we train with ids to reduce training time
        ...
        return (heads, rels, tails) # return the 3 np.arrays together in a matrix

    # triple with class
    def read_triple_class(self, path, mode, skip_first_line=False):
        # mode: train/valid/test
        if path is None:
            return None

        print('Reading {} triples with class....'.format(mode)) # e.g. Reading train triples with class...
        heads = []
        tails = []
        rels = []
        classes = []
        with open(path) as f:
            ...
            for line in f:
                triple = line.strip().split(self.delimiter)
                h, r, t, classe = triple[0], triple[1], triple[2], triple[3]
                heads.append(self.entity2id[h]) # self.entity2id is already called before self.read_triple_class
                rels.append(self.relation2id[r]) # so we can get the id of the head/relation/tail
                tails.append(self.entity2id[t]) # we train with ids to reduce training time
                classes.append(classe) # we add class for this triple

        heads = np.array(heads, dtype=np.int64)
        tails = np.array(tails, dtype=np.int64)
        rels = np.array(rels, dtype=np.int64)
        classes = np.array(classes, dtype=np.int64)
        print('Finished. Read {} {} triples with class.'.format(len(heads), mode))

        return (heads, rels, tails, classes) # return the 4 np.arrays together in a matrix
...
class KGDatasetFB15k(KGDataset):
    ...
    * test.txt stores the triples in the test set.
    ...
    def __init__(self, path, name='FB15k'):
        self.name = name
        url = 'https://data.dgl.ai/dataset/{}.zip'.format(name) # download from dgl
        ...
        super(KGDatasetFB15k, self).__init__(os.path.join(self.path, 'entities.dict'),
                                             ...
                                             os.path.join(self.path, 'train_class_map.txt'))
...
# My Datasets
class KGDatasetYAGO3_10(KGDataset):
    '''Load a knowledge graph YAGO3-10

    The YAGO3-10 dataset has five files:
    * entities.dict stores the mapping between entity Id and entity name.
    * relations.dict stores the mapping between relation Id and relation name.
    * train.txt stores the triples in the training set.
    * valid.txt stores the triples in the validation set.
    * test.txt stores the triples in the test set.
    ...
    The triples are stored as 'head_nid\trelation_id\ttail_nid'.
    '''
    def __init__(self, path, name='YAGO3_10'):
        self.name = name
        ...
        super(KGDatasetYAGO3_10, self).__init__(os.path.join(self.path, 'entities.dict'),
                                             os.path.join(self.path, 'relations.dict'),
                                             os.path.join(self.path, 'train.txt'),
                                             os.path.join(self.path, 'valid.txt'),
                                             os.path.join(self.path, 'test.txt'),
...
class KGDatasetYAGO4_10(KGDataset):
    '''Load a knowledge graph YAGO4-10

    The YAGO4-10 dataset has five files:
    * entities.dict stores the mapping between entity Id and entity name.
    * relations.dict stores the mapping between relation Id and relation name.
    * train.txt stores the triples in the training set.
    * valid.txt stores the triples in the validation set.
    * test.txt stores the triples in the test set.
    ...
    The triples are stored as 'head_nid\trelation_id\ttail_nid'.
    '''
    def __init__(self, path, name='YAGO4_10'):
        self.name = name
        #url = 'https://data.dgl.ai/dataset/{}.zip'.format(name)
        ...
        super(KGDatasetYAGO4_10, self).__init__(os.path.join(self.path, 'entities.dict'),
                                             os.path.join(self.path, 'relations.dict'),
                                             os.path.join(self.path, 'train.txt'),
                                             os.path.join(self.path, 'valid.txt'),
                                             os.path.join(self.path, 'test.txt'),
...
class KGDatasetUDDRaw(KGDataset):
    ...
    * test stores the triples in the test set. In format [src_name, rel_name, dst_name]
    ...
    def __init__(self, path, name, delimiter, files, format, has_edge_importance=False):
        ...
        assert len(format) == 3 # We use format == "hrt"
        format = _parse_srd_format(format)
        
        self.load_entity_relation(path, delimiter, files, format) # call the function and write map files
    ...
    def read_triple(self, path, mode, skip_first_line=False, format=[0,1,2]): # never called ??
        # mode: train/valid/test
        if path is None:
            return None
    ...
    # Same as read_entity + read_relation in KGDataset + write map files
    def load_entity_relation(self, path, delimiter, files, format):
        entity_map = {}
        rel_map = {}
        for fi in files: # 1 or 3 files, train, valid, test
            with open(os.path.join(path, fi)) as f:
                for line in f:
                    ...
                    src_id = _get_id(entity_map, src) # add new key-value pairs in entity_map
                    dst_id = _get_id(entity_map, dst) # add new key-value pairs in entity_map
                    rel_id = _get_id(rel_map, rel) # add new key-value pairs in rel_map

        entities = ["{}{}{}\n".format(val, delimiter, key) for key, val in entity_map.items()]
        with open(os.path.join(path, "entities.tsv"), "w+") as f:
            f.writelines(entities) # write entity_map in the file
        ...
        with open(os.path.join(path, "relations.tsv"), "w+") as f:
            f.writelines(relations) # write rel_map in the file
        self.relation2id = rel_map
        self.n_relations = len(relations)

    def read_entity(self, entity_path): # we can only read after load
    ...
    def read_triple(self, path, mode, skip_first_line=False, format=[0,1,2]):
        ...
        with open(path) as f:
            ...
            for line in f:
                ...
                h, r, t = triple[format[0]], triple[format[1]], triple[format[2]]
                try:
                    heads.append(int(h)) # triples are already ids
                    tails.append(int(t))
                    rels.append(int(r))
                    if self.has_edge_importance:
                        e_impts.append(float(triple[3]))
...
def get_dataset(data_path, data_name, format_str, delimiter='\t', files=None, has_edge_importance=False):
    if format_str == 'built_in':
        ...
        # our datasets
        elif data_name == 'YAGO3_10':
            dataset = KGDatasetYAGO3_10(data_path)
        elif data_name == 'YAGO4_10':
            dataset = KGDatasetYAGO4_10(data_path)
        else:
        ...
        format = format_str[8:] # after raw_udd_
        ...
        format = format_str[4:] # after udd_
