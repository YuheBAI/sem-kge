...
# sampler.py
#
# This file is modified, add function SemanticPartition
#
...
def SemanticPartition(edges, n, has_importance=False, threshold=0.05):
    """ This partitions a list of edges to n partitions according to the
    class types of entities s. 
    For any class with number of edges(according to s) larger than the
    threshold, its edges will be evenly distributed into all partitions.
    For any class with number of edges smaller than the threshold, its
    edges will be put into one single partition.


    Parameters
    ----------
    entity2classse : dictionary
        key is entity id, value is its classe id
    edges : (heads, rels, tails, class) triple
        Edge list to partition
    n : int
        Number of partitions
    threshold : float
        The threshold of whether a class is LARGE or SMALL
        Default: 5%
    ...
    bool
        Whether there exists some classes belongs to multiple partitions
    """

    if has_importance:
        heads, rels, tails, classes, e_impts = edges
    else:
        heads, rels, tails, classes = edges # now we have 4 values
    print('semantic partition {} edges into {} parts'.format(len(heads), n))
    uniq, cnts = np.unique(classes, return_counts=True) # count the number of edges for each class
    idx = np.flip(np.argsort(cnts)) # sort the index of edge counts for each class from max to min, e.g. [4325,5,2,4,0,1]
    cnts = cnts[idx] # sort the edge count for each class from max to min
    uniq = uniq[idx] # sort the class id from max to min (frequency)
    assert cnts[0] > cnts[-1]
    edge_cnts = np.zeros(shape=(n,), dtype=np.int64) # edge counts for each partition
    class_cnts = np.zeros(shape=(n,), dtype=np.int64) # class counts for each partition
    class_dict = {}
    class_parts = [] # list of length n with class id of each partition
    cross_class_part = []
    for _ in range(n):
        class_parts.append([])
    ...
    for i in range(len(cnts)):
        ...
        if cnt > large_threshold:
            ...
            for j in range(n):
                ...
                c_parts.append([j, part_cnt])
                class_parts[j].append(c)
                edge_cnts[j] += part_cnt
                class_cnts[j] += 1
                cnt -= part_cnt
            cross_class_part.append(c)
        else:
            idx = np.argmin(edge_cnts)
            c_parts.append([idx, cnt])
            class_parts[idx].append(c)
            edge_cnts[idx] += cnt
            class_cnts[idx] += 1
        class_dict[c] = c_parts

    for i, edge_cnt in enumerate(edge_cnts):
        print('part {} has {} edges and {} classes'.format(i, edge_cnt, class_cnts[i]))
    print('{}/{} duplicated classes across partitions'.format(num_cross_class, len(cnts)))

    parts = []
    for i in range(n):
        parts.append([])
        class_parts[i] = np.array(class_parts[i])

    for i, c in enumerate(classes):
        c_part = class_dict[c][0]
        part_idx = c_part[0]
        cnt = c_part[1]
    ...
    for i, part in enumerate(parts):
        ...
        off += len(part)
    cross_class_part = np.array(cross_class_part)

    return parts, class_parts, num_cross_class > 0, cross_class_part
...
def SoftRelationPartition(edges, n, has_importance=False, threshold=0.05):
    ...
    print("soft relation partition function is called")
    print('relation partition {} edges into {} parts'.format(len(heads), n))
    uniq, cnts = np.unique(rels, return_counts=True) # count the number of edges for each relation
    idx = np.flip(np.argsort(cnts)) # sort the index of edge counts for each relation from max to min, e.g. [3,5,2,4,0,1]
    cnts = cnts[idx] # sort the edge count for each relation from max to min
    uniq = uniq[idx] # sort the relation name from max to min
    assert cnts[0] > cnts[-1] # ??????????
    edge_cnts = np.zeros(shape=(n,), dtype=np.int64) # edge counts for each partition
    rel_cnts = np.zeros(shape=(n,), dtype=np.int64) # rel counts for each partition
    rel_dict = {}
    rel_parts = [] # list of length n with relation names of each partition
    cross_rel_part = [] # list of relation names that belong to n partitions
    for _ in range(n):
        rel_parts.append([]) # empty list of length n

    large_threshold = int(len(rels) * threshold) # number of edges(triples)
    capacity_per_partition = int(len(rels) / n)
    # ensure any relation larger than the partition capacity will be split
    large_threshold = capacity_per_partition if capacity_per_partition < large_threshold \
                      else large_threshold # if core > 20 for threshold=0.05, large threshold = len/n
    num_cross_part = 0 # number of large relations
    for i in range(len(cnts)): # number of unique relations from largest relation, e.g. 92 for YAGO4-10
        cnt = cnts[i] # number of edges for current relation
        r = uniq[i] # current relation name
        r_parts = [] # List of np.array: nb Edges of each partition
        if cnt > large_threshold: # will split into n partitions
            avg_part_cnt = (cnt // n) + 1 # number of edges for each partition
            num_cross_part += 1
            for j in range(n):
                part_cnt = avg_part_cnt if cnt > avg_part_cnt else cnt # avg_part_cnt in general, cnt for last partition
                r_parts.append([j, part_cnt]) # nb edges of current r for each partition * each relation
                rel_parts[j].append(r) # append current relation name to all partitions(since it's split)
                edge_cnts[j] += part_cnt # edge counts for each partition
                rel_cnts[j] += 1 # rel counts for each partition
                cnt -= part_cnt # number of remained edges for current relation
            cross_rel_part.append(r)
        else: # cnt < large_threshold
            idx = np.argmin(edge_cnts) # find the most empty partition
            r_parts.append([idx, cnt]) # append nb edges to this partition
            rel_parts[idx].append(r) # append current relation name to this partition
            edge_cnts[idx] += cnt
            rel_cnts[idx] += 1
        rel_dict[r] = r_parts 
        """ key is relation name, value is a list of tuples. 
        Each tuple contains two integers: a partition index and the number of edges for the relation in that partition.
        e.g. "livesIn": [(0, 5), (1, 3), (2, 2)] means the relation "livesIn" is split into three partitions, 
        with 5 edges in the first partition, 3 edges in the second partition, and 2 edges in the third partition.
        """

    for i, edge_cnt in enumerate(edge_cnts): # edge counts for each partition
    ...
    parts = [] # a list of n arrays, include all the edges index of n partitions
    for i in range(n):
        parts.append([])
        rel_parts[i] = np.array(rel_parts[i]) # list of n np.array

    for i, r in enumerate(rels): # time consuming, to order all triples of the whole graph
        r_part = rel_dict[r][0] # take the first tuple for current relation, from partition 1, 2 to n
        part_idx = r_part[0] # partition index
        cnt = r_part[1] # nb of edges remained in this partition for this relation
        parts[part_idx].append(i) # append this edge index
        cnt -= 1 # added 1 edge
        if cnt == 0: # if no place remained in this partition for this relation
            rel_dict[r].pop(0) # removes the first tuple of this relation, from partition 1, 2 to n
        else:
            rel_dict[r][0][1] = cnt # remained place -1

    
    for i, part in enumerate(parts): # n partitions
        parts[i] = np.array(part, dtype=np.int64) # convert to np.array
    shuffle_idx = np.concatenate(parts) # concatenate all the np arrays in parts into a single np array
    heads[:] = heads[shuffle_idx] # shuffle the elements in the heads, rels, tails
    rels[:] = rels[shuffle_idx]
    tails[:] = tails[shuffle_idx] # is it changed after return ?????
    if has_importance:
        e_impts[:] = e_impts[shuffle_idx]
    ...
    for i, part in enumerate(parts):
        parts[i] = np.arange(off, off + len(part))
        off += len(part)
    cross_rel_part = np.array(cross_rel_part) # convert to np.array

    """
    - parts: a list of n arrays, include the order information of n partitions
    - rel_parts contains the relation names of all relations
    - cross_rel_part: relations that belong to multiple partitions
    """
...
def BalancedRelationPartition(edges, n, has_importance=False):
    ...
    if has_importance:
        ...
        heads, rels, tails = edges
    print("balanced relation partition function is called")
    print('relation partition {} edges into {} parts'.format(len(heads), n))
...
def RandomPartition(edges, n, has_importance=False):
    ...
    if has_importance:
        ...
        heads, rels, tails = edges
    print("random partition function is called")
    print('random partition {} edges into {} parts'.format(len(heads), n))
    idx = np.random.permutation(len(heads)) # we permute randomly the order
    heads[:] = heads[idx] # change heads by order
    rels[:] = rels[idx] # change rels by order
    tails[:] = tails[idx] # change tails by order
    if has_importance:
        e_impts[:] = e_impts[idx]

    part_size = int(math.ceil(len(idx) / n)) # Ceiling of len/n
    parts = [] # a list of n arrays
    for i in range(n):
        start = part_size * i # 0 for first partition, part_size for second, etc.
        end = min(part_size * (i + 1), len(idx)) # len for the last partition
        parts.append(idx[start:end]) # append the order
        print('part {} has {} edges'.format(i, len(parts[-1]))) # length of the last appended array
    return parts # a list of n arrays, include the order information of n partitions


def ConstructGraph(dataset, args): # dataset contains SPOs
    ...
    args :
        Global configs.
    """
    src = [dataset.train[0]] # heads
    etype_id = [dataset.train[1]] # relations
    dst = [dataset.train[2]] # tails
    ...
    if args.dataset == "wikikg90M":
        ...
        src.append(dataset.valid[0]) # append valid set
    ...
    if args.dataset == "wikikg90M":
        ...
        src.append(dataset.test[0]) # append test set
    ...
    src = np.concatenate(src) # concatenate train, valid, test to 1 array
    ...
    #g = dgl.graph((src, dst), num_nodes=n_entities) # new dgl version
    #g = dgl.from_scipy(coo) # new dgl version
    g.edata['tid'] = F.tensor(etype_id, F.int64) # edge information: relation_id, put it automatically by order
    ...
    #g = dgl.to_homogeneous(g, edata=['tid']) # convert to homogeneous graph

    return g # type: <class 'dgl.heterograph.DGLHeteroGraph'>
...
class TrainDataset(object):
    ...
    def __init__(self, g, dataset, args, ranks=64, has_importance=False):
        ...
        triples_with_class = dataset.train_with_class
        # entity2classe = dataset.entity2classe
        num_train = len(triples[0])
        print('|Train|:', num_train)

        if ranks > 1 and args.rel_part:
            self.edge_parts, self.rel_parts, self.cross_part, self.cross_rels = \
            SoftRelationPartition(triples, ranks, has_importance=has_importance) # print
        elif ranks > 1 and args.sem_part:
            self.edge_parts, self.rel_parts, self.cross_part, self.cross_rels = \
            SemanticPartition(triples_with_class, ranks, has_importance=has_importance) # print
        elif ranks > 1:
            self.edge_parts = RandomPartition(triples, ranks, has_importance=has_importance) # print
    ...
    def create_sampler(self, batch_size, neg_sample_size=2, neg_chunk_size=None, mode='head', num_workers=32,
        ...
        EdgeSampler = getattr(dgl.contrib.sampling, 'EdgeSampler') # new dgl version
...
class EvalSampler(object):
    ...
    def __init__(self, g, edges, batch_size, neg_sample_size, neg_chunk_size, mode, num_workers=32,
        ...
        # new version: dgl.dataloading.EdgeDataLoader
        EdgeSampler = getattr(dgl.contrib.sampling, 'EdgeSampler') # new version
